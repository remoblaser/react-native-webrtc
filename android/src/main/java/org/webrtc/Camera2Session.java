...
class Camera2Session implements CameraSession {
  ...
  private static final Histogram camera2ResolutionHistogram = Histogram
      .createEnumeration("WebRTC.Android.Camera2.Resolution", CameraEnumerationAndroid.COMMON_RESOLUTIONS.size());
  ...
  private class CaptureSessionCallback extends CameraCaptureSession.StateCallback {
    ...
    public void onConfigured(CameraCaptureSession session) {
      ...
      try {
        /*
         * The viable options for video capture requests are: TEMPLATE_PREVIEW: High
         * frame rate is given priority over the highest-quality post-processing.
        ...
        captureRequestBuilder.set(CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, new Range<Integer>(
            captureFormat.framerate.min / fpsUnitFactor, captureFormat.framerate.max / fpsUnitFactor));
      ...
      surfaceTextureHelper.startListening((VideoFrame frame) -> {
        ...
        final VideoFrame modifiedFrame = new VideoFrame(
            CameraSession.createTextureBufferWithModifiedTransformMatrix((TextureBufferImpl) frame.getBuffer(),
                /* mirror= */ isCameraFrontFacing, /* rotation= */ -cameraOrientation),
    ...
    private void chooseStabilizationMode(CaptureRequest.Builder captureRequestBuilder) {
      final int[] availableOpticalStabilization = cameraCharacteristics
      ...
      final int[] availableVideoStabilization = cameraCharacteristics
  ...
  public static void create(CreateSessionCallback callback, Events events, Context applicationContext,
      CameraManager cameraManager, SurfaceTextureHelper surfaceTextureHelper, String cameraId, int width, int height,
      int framerate) {
    new Camera2Session(callback, events, applicationContext, cameraManager, surfaceTextureHelper, cameraId, width,
        height, framerate);
  }
